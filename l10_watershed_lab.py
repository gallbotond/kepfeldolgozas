...
def watershed():
        ...
        for y in range(imBe.shape[1]):
            ...
            if q < fp:
                for irany in range(8):
                    if x + dx[irany] >= 0 and x + dx[irany] < imBe.shape[0] and \
                        y + dy[irany] >= 0 and y + dy[irany] < imBe.shape[1]:
    ...
    while nextOut < nextIn:
        ...
        fp = imG[px, py]
        for irany in range(8):
            if px + dx[irany] >= 0 and px + dx[irany] < imBe.shape[0] and \
                        py + dy[irany] >= 0 and py + dy[irany] < imBe.shape[1]:
    ...
    for x in range(imBe.shape[0]):
        for y in range(imBe.shape[1]):
            ...
            while nrStack > 0:
                ...
                pvout = imKi[pvx, pvy]
                for irany in range(8):
                    if pvx + dx[irany] >= 0 and pvx + dx[irany] < imBe.shape[0] and \
                        pvy + dy[irany] >= 0 and pvy + dy[irany] < imBe.shape[1]:
                        fpvv = imG[pvx + dx[irany], pvy + dy[irany]]
                        pvvout = imKi[pvx + dx[irany], pvy + dy[irany]]
                        if fpv == fpvv and pvvout == 8 and not(pvx + dx[irany] == x and pvy + dy[irany] == y):
    ...
    for x in range(imBe.shape[0]):
        for y in range(imBe.shape[1]):
            ...
            while nrStack > 0:
                ...
                o += np.uint32(round(float(r * .299) + float(g * .587) + float(b * .114)) * 0x1000000)
                
                medbuf.append(o)
                pvin = imBe[pvx, pvy]
                for irany in range(8):
                    if (bits[irany] & pvin) > 0 and not ((pvx + dx[(irany + 4) % 8], pvy + dy[(irany + 4) % 8]) in fifo[:nextIn]) and \
                        pvx + dx[(irany + 4) % 8] > 0 and pvx + dx[(irany + 4) % 8] < imBe.shape[0] and pvy + dy[(irany + 4) % 8] > 0 and pvy + dy[(irany + 4) % 8] < imBe.shape[1]:
                        stack[nrStack] = (pvx + dx[(irany + 4) % 8], pvy + dy[(irany + 4) % 8])
                        nrStack += 1
            label += 1
            if nextIn < 2:
                print(nextIn)
